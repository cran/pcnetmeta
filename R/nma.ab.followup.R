nma.ab.followup <-
function(s.id,t.id,event.n,total.n,followup,trtname,model="het_cor",prior.type,a=0.001,b=0.001,c=10,param=c("rate","logratio","best"),higher.better=FALSE,digits=4,n.adapt=5000,n.iter=100000,n.burnin=floor(n.iter/2),n.chains=3,n.thin=max(1,floor((n.iter-n.burnin)/100000)),conv.diag=FALSE,trace="",dic=FALSE,postdens=FALSE){
  ...
  ## jags model
  if(model=="hom"){
    model.followup.hom(prior.type)
  }
  if(model=="het_ind"){
    model.followup.het.ind(prior.type)
  }
  if(model=="het_cor"){
    I <- diag(ntrt)
    model.followup.het.cor(prior.type)
  }

  ## jags data
  if(model == "hom"| model == "het_ind"){
    if(prior.type == "unif"){
      data.jags<-list(s=s.id,t=t.id,y=event.n,totaln=total.n,f=followup,len=len,nstudy=nstudy,ntrt=ntrt,c=c,higher.better=higher.better)
    }
    if(prior.type == "invgamma"){
      data.jags<-list(s=s.id,t=t.id,y=event.n,totaln=total.n,f=followup,len=len,nstudy=nstudy,ntrt=ntrt,a=a,b=b,higher.better=higher.better)
    }
  }
  if(model=="het_cor"){
    data.jags<-list(s=s.id,t=t.id,y=event.n,totaln=total.n,f=followup,len=len,nstudy=nstudy,ntrt=ntrt,zeros=rep(0,ntrt),I=I,higher.better=higher.better)
  }
