...
function(s.id,t.id,event.n,total.n,followup,data,trtname,param=c("lograte","logratio","rank.prob"),model="het_cor",prior.type,a=0.001,b=0.001,c=10,higher.better=FALSE,digits=4,n.adapt=5000,n.iter=100000,n.burnin=floor(n.iter/2),n.chains=3,n.thin=max(1,floor((n.iter-n.burnin)/100000)),conv.diag=FALSE,trace="",dic=FALSE,postdens=FALSE){
  ...
  ## jags model
  if(model=="hom_ind"){
    model.followup.hom.ind(prior.type,is.element("rank.prob",param))
  }
  if(model=="het_ind"){
    model.followup.het.ind(prior.type,is.element("rank.prob",param))
  }
  if(model=="hom_eqcor"){
    model.followup.hom.eqcor(prior.type,is.element("rank.prob",param))
  }
  if(model=="het_eqcor"){
    model.followup.het.eqcor(prior.type,is.element("rank.prob",param))
  }
  if(model=="het_cor"){
    I <- diag(ntrt)
    model.followup.het.cor(prior.type,is.element("rank.prob",param))
  }
